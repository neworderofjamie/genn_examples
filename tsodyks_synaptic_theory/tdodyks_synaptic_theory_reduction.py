...
# Network parameters
# ==================
# Fraction of excitatory neurons in each selective pop (0.1)
F = 0.25

# Number of selective memories (5)
P = 2

# What fraction of total neurons are presynaptic to each other neuron (0.2)
C = 0.2
N_E = 3200
N_I = 800
print(f"{N_E} excitatory neurons {N_I} inhibitory neurons")

WEIGHT_SCALE = 10000 / (N_E + N_I)
...
print(f"{N_E_SELECT} neurons in each {P} selective populations and {N_E_NON_SELECT} in non-selective population")
...
model = GeNNModel("float", "tsodyks_synaptic_theory_reduction")
model.dt = 0.1
model.seed = 1234
...
# Load
model.add_current_source(f"LoadCurr", stim_model, e_select_pop[1],
                         {"mean": MU_LOAD, "sd": 0.0,
                          "startTime": 3000.0, "endTime": 3000.0 + 350.0},
                         {"current": 0.0})

#model.add_current_source(f"LoadCurr5", stim_model, e_select_pop[4],
#                         {"mean": MU_LOAD, "sd": 0.0,
#                          "startTime": 3800.0, "endTime": 3800.0 + 350.0},
#                         {"current": 0.0})
                             
# Inhibitory synapse populations
# ==============================
num_inh_pre = int(C * N_I)
print(f"{num_inh_pre} inhibitory inputs per neuron")
add_synapse_pop(model, i_pop, i_pop, J_II * WEIGHT_SCALE, num_inh_pre)
add_synapse_pop(model, i_pop, e_non_select_pop, J_IE * WEIGHT_SCALE, num_inh_pre)
for e in e_select_pop:
    add_synapse_pop(model, i_pop, e, J_IE * WEIGHT_SCALE, num_inh_pre)
...
print(f"{num_non_select_num_pre} non-selective excitatory inputs per neuron")
add_synapse_pop(model, e_non_select_pop, i_pop, 
                J_EI * WEIGHT_SCALE, num_non_select_num_pre)

non_select_g_init = init_var(non_selective_init, {"Jb": J_B * WEIGHT_SCALE, 
                                                  "Jp": J_P * WEIGHT_SCALE,
                                                  "p": 0.1})
...
print(f"{num_select_num_pre} selective excitatory inputs per neuron")
for e_pre in e_select_pop:
    add_synapse_pop(model, e_pre, i_pop, J_EI * WEIGHT_SCALE, num_select_num_pre)
    add_stp_synapse_pop(model, e_pre, e_non_select_pop, J_B * WEIGHT_SCALE, num_select_num_pre)
    
    for e_post in e_select_pop:
        j = J_P if e_pre == e_post else J_B * 4.0
