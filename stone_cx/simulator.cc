...
int main()
{
    ...
    VonMisesDistribution<double> pathVonMises(0.0, SimParameters::pathKappa);
    ...
    tk::spline accelerationSpline;
    {
        ...
        const unsigned int numAccelerationChanges = SimParameters::numOutwardTimesteps / 50;
        ...
        std::uniform_real_distribution<double> acceleration(SimParameters::agentMinAcceleration,
                                                            SimParameters::agentMaxAcceleration);
    ...
    for(unsigned int i = 0; i < (SimParameters::numOutwardTimesteps + SimParameters::numInwardTimesteps); i++) {
        ...
        const bool outbound = (i < SimParameters::numOutwardTimesteps);
        double a = 0.0;
        if(outbound) {
            // Update angular velocity
            omega = (SimParameters::pathLambda * omega) + pathVonMises(gen);
        ...
        // Otherwise we're path integrating home
            ...
            omega = -SimParameters::agentM * (rightMotor - leftMotor);
        ...
        xVelocity -= SimParameters::agentDrag * xVelocity;
        yVelocity -= SimParameters::agentDrag * yVelocity;
